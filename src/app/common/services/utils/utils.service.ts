...
export class UtilsService {
  ...
  convertToMomentArray(
    ...
    switch (this.getInputType(value, allowMultiSelect)) {
      case ECalendarValue.String: {
        const _a = value ? [moment.from(<string>value, "fa")] : [];
        return _a;
      }
      case ECalendarValue.StringArr: {
        const _b = (<string[]>value)
          .map((v) => (v ? moment(v, format, true).locale(locale) : null))
          .filter(Boolean);
        return _b;
      }
      case ECalendarValue.Moment: {
        const _c = value ? [(<Moment>value).clone().locale(locale)] : [];
        return _c;
      }
      case ECalendarValue.MomentArr: {
        const _d = (<Moment[]>value || []).map((v) => v.clone().locale(locale));
        return _d;
      }
      default:
        return [];
  ...
  convertFromMomentArray(
    ...
    switch (convertTo) {
      ...
      case ECalendarValue.StringArr:
        return value
          .filter(Boolean)
          .map((v) => v.locale(locale).format(format));
      case ECalendarValue.Moment:
  ...
  updateSelected(
    ...
    date: IDate,
    granularity: unitOfTime.Base = "day"
  ): Moment[] {
  ...
  createValidator(
    ...
    return (inputVal: CalendarValue) => {
      ...
      value = this.convertToMomentArray(inputVal, format, true, locale).filter(
        Boolean
      );
